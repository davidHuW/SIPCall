...
void CReportCtrl::SetNoEditCol(int* pNoEditCol, int nNoEditColNum)
{
	m_pNoEditCol = pNoEditCol;
	m_nNoEditColNum = nNoEditColNum;
}
bool CReportCtrl::IsNoEditCol(int col)
{
	if (!m_bAllowEdit)
	{
		return false;
	}
	for (int i = 0; i < m_nNoEditColNum; i++)
	{
		if (col == m_pNoEditCol[i])
		{
			return true;
		}
	}
