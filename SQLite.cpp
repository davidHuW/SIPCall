...
#ifdef  UNICODE    
// 	char pSQL[1024] = {0};
// 	WideCharToMultiByte(CP_ACP, 0, lpSql, -1, pSQL, 1024, NULL, NULL);
    if(sqlite3_prepare16_v2(m_db, /*pSQL*/lpSql, -1, &stmt, NULL) != SQLITE_OK)  
#else   
    if(sqlite3_prepare_v2(m_db, lpSql, -1, &stmt, NULL) != SQLITE_OK)  
#endif   
    {   
...
bool SQLiteDataReader::IsExistColumn(LPCTSTR columnName)
{
	int allCount = ColumnCount();
	for (int i = 0; i < allCount; i++)
	{
		LPCTSTR name = GetName(i);
		int  flag = -1;
#ifdef _UNICODE
		flag = wcscmp(name ,columnName);
#else
		flag = strcmp(name ,columnName);
#endif
		if (flag == 0)
		{
			return true;
